...
int main(int, char *argv[]) {
  ...
  donepp:
    ...
    for (long long i = std::sqrt(1e11) - 2;; --i)
    ...
    std::vector<long long> ns;
    for (long long i = (std::sqrt(1e11) - 1) / 2;; --i)
    ...
    std::vector<long long> ns;
    long long L = std::sqrt((std::sqrt(1e11) - 2));
    for (long long j = L;; --j)
      if (prime::is_prime(j)) {
