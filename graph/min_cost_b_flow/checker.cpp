    ...
    for (size_t i = 0; i < m; ++i) {
        ...
        if (f < e.lower) {
            quitf(_wa, "Capacity constraint:"
                   "The given flow on %lld-th edge (" I64 ") is lower than the lower bound of the edge (" I64 ")",
                   (unsigned long long int) i, f, e.lower);
        }
        if (f > e.upper) {
            quitf(_wa, "Capacity constraint: "
                   "The given flow on %lld-th edge (" I64 ") is greater than the upper bound of the edge (" I64 ")",
        ...
        if ((f > e.lower) && (reduced_cost > 0)) {
            quitf(_wa, "Complementary slackness condition: "
                   "Flow on %lld-th edge (" I64 ") is greater than the lower bound (" I64 "), but the reduced cost (" I64 " = " I64 " + " I64 " - " I64 ") is positive",
                   (unsigned long long int) i, f, e.lower, reduced_cost, e.cost, potentials[e.s], potentials[e.t]);
        }
        if ((f < e.upper) && (reduced_cost < 0)) {
            quitf(_wa, "Complementary slackness condition: "
                   "Flow on %lld-th edge (" I64 ") is less than the upper bound (" I64 "), but the reduced cost (" I64 " = " I64 " + " I64 " - " I64 ") is negative",
    ...
    for (size_t v = 0; v < n; ++v) {
        if (bs[v] != b_from_flow[v]) {
            quitf(_wa, "Flow conservation constraint: "
                   "%lld-th vertex should supply " I64 " flow but was " I64 "",
