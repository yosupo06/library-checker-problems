...
template <typename T> FormalPowerSeries<T> diff(const FormalPowerSeries<T> &f) {
  int n = (int)f.size();
  FormalPowerSeries<T> res(n - 1, 0);
  for (int i = 1; i < n; i++)
    res[i - 1] = f[i] * i;
...
FormalPowerSeries<T> pow(const FormalPowerSeries<T> &f, ll e, int deg = -1) {
  ...
  while (i < (int)f.size() && f[i] == 0 && i * e < deg)
    i++;
  if (i == (int)f.size())
    return FormalPowerSeries<T>(deg, 0);
  if (i * e >= deg)
    return FormalPowerSeries<T>(deg, 0);
  T k = f[i];
