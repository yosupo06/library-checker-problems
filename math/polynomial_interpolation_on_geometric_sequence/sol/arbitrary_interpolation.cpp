...
struct InterPol {
  ...
  InterPol(const V<Mint>& _que) {
    ...
    for (int i = 0; i < n; i++) { que[0][i] = {{-_que[i], 1}}; }
  ...
  Po query(const V<Mint>& ys) const {
    V<Po> res(n);
    for (int i = 0; i < n; i++) { res[i] = {{ys[i] / s.freq(i)}}; }
...
int main() {
  ...
  V<Mint> x(n), y(n);
  if (n > 0) x[0] = a;
  for (int i = 0; i < n - 1; ++i) x[i + 1] = x[i] * r;

  for (int i = 0; i < n; i++) { scanf("%d", &y[i].v); }

  Poly<Mint> pol = (n > 0 ? InterPol<Mint>(x).query(y) : Poly<Mint>{});
