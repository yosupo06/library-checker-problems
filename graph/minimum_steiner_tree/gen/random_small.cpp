...
int main(int, char *argv[]) {
  ...
  int N = gen.uniform(1, (int)N_SMALL_MAX);
  int M = gen.uniform(N - 1, (int)M_MAX);

  vector<E> E;
  for(int i = 1; i < N; i++)
    E.push_back({gen.uniform(0, i - 1), i, (int)gen.uniform(W_MIN, W_MAX)});
  for(int i = N - 1; i < M; i++) {
    int Ui = gen.uniform(0, N - 1);
    int Vi = gen.uniform(0, N - 1);
    int Wi = gen.uniform(W_MIN, W_MAX);
    E.push_back({Ui, Vi, Wi});
  }

  int K = gen.uniform(1, min(N, (int)K_MAX));
