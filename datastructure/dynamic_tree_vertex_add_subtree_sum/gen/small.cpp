...
int main(int, char* argv[]) {
    ...
    int n = gen.uniform(900, 1000);
    int q = gen.uniform(900, 1000);
    printf("%d %d\n", n, q);
    for (int i = 0; i < n; i++) {
        printf("%lld", gen.uniform(A_AND_X_MIN, 10ll));
        if (i != n - 1) printf(" ");
    }
    printf("\n");
    ...
    for (int i = 0; i < q; i++) {
        ...
        if(t == 0) {
          ...
          printf("%d %d %d %d\n", u, v, w, x);
            ...
            int x = gen.uniform(A_AND_X_MIN, 10ll);
            printf("%d %d\n", p, x);
        } else {
            auto eidx = gen.uniform(0, n - 2);
            auto p = tree.edges[eidx];
            if(gen.uniform_bool()) {
