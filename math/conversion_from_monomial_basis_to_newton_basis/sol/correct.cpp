...
std::vector<Mint> monomial_to_newton(int n, Poly f, std::vector<Mint> p) {
    ...
    Poly g = mul(f, fps_inv(pr, s));
    g.resize(s);
    std::reverse(g.begin(), g.end());
    std::vector<Poly> h(2 * s);
    h[1] = g;
    for (int i = 1; i < s; ++i) {
        int sz = (int)prod[2 * i + 1].size() - 1;
        Poly p = mul(h[i], prod[2 * i + 1]);
        h[2 * i] = Poly(p.begin() + sz, p.begin() + 2 * sz);
        h[2 * i + 1] = Poly(h[i].begin() + sz, h[i].end());
    }
    std::vector<Mint> ans(s);
    for (int i = 0; i < s; ++i) {
        ans[i] = h[s + i][0];
    }
    ans.resize(n);
