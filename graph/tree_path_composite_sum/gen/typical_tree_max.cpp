...
struct Tree {
    ...
    static Tree GenerateByTemplateId(Random& rng, int id, int n){
        ...
        if(id-- == 0){ auto res = Line(n/2); res.toCentipede(); res.join_random_edges(rng, n%2); return res; }
        ...
        if(id-- == 0){ auto res = Star(n/2); res.expand_random_edges(rng, n/2); return res; }
        assert(false);
    }
