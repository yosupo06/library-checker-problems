...
int main(int, char* argv[]) {
    ...
    int n = gen.uniform<int>(std::max<int>(N_MIN, N_MAX/10), N_MAX);
    int m = gen.uniform<int>(std::max<int>(M_MIN, M_MAX/10), M_MAX);
    printf("%d %d\n", n, m);

    int arti_count = 3 + gen.uniform<int>(0, 100);
    int involved_vtxs = 3 + gen.uniform<int>(arti_count, n);
    auto edges = bct_to_edges(gen, generate_bct(gen, arti_count), involved_vtxs, m);
