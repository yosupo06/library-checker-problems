...
int main() {
  int N;
  scanf("%d", &N);
  // log(1 + x) = sum_{k >= 1} (-1)^{k - 1} x^k / k
  // In EGF form: log(1 + x) = sum_{k >= 0} f_k x^k / k!, where
  // f_0 = 0, f_k = (-1)^{k - 1} (k - 1)! (k >= 1).
  std::vector<mint> f(N + 1, 0), s(1 << N);
  mint fac = 1; // fac = (k - 1)!
  for (int k = 1; k <= N; k++) {
    f[k] = (k & 1) ? fac : 0 - fac;
    fac *= k;
