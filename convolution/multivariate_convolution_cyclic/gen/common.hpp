...
std::vector<int> rand_vec(const std::vector<int> &ns, Random &gen, int mod) {
  ...
  for (int n: ns) N *= n;
  std::vector<int> v(N);
  for (int i = 0; i < N; i++) { v[i] = gen.uniform(0, mod - 1); }
  return v;
}
...
int rand_prime(const std::vector<int> &ns, Random &gen) {
  int lcm = 1;
  for (int n: ns) { lcm = lcm / std::gcd(n, lcm) * n; }
  int N = lcm;
  int p = 0;
  while (1) {
    p = 1 + N * gen.uniform<int>(1, (P_MAX - 1) / N);
