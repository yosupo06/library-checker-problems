...
std::vector<uint> FPSInv(const std::vector<uint> &a, int n) {
  ...
  for (int i = 2; i <= N; i *= 2) {
    ...
    InvFFT(data(shopA), i, data(inv_root));
    for (int j = i / 2; j < i; ++j)
      invA[j] = shopA[j] != 0 ? MOD - shopA[j] : shopA[j];
  }
  invA.resize(n);
  return invA;
}
...
int main() {
  ...
  for (int i = n - 1; i > 0; --i)
    inv_factorial[i] = (ull)inv_factorial[i + 1] * (i + 1) % MOD;

  std::vector<uint> A(n);
  for (int i = 0; i < n; ++i)
    A[i] = (ull)f[i] * factorial[i] % MOD;

  std::vector<uint> B(n);
  for (int i = 0; i < n; ++i)
    B[i] = inv_factorial[i + 1];
  B = FPSInv(B, n);
  ...
  // ...
  // Faulhaber's formula:
  // S_k(n) := 1/(k+1) * Σ[1 <= j <= k+1] (-1)^delta(j, k) * binom(k+1, j) *
  // B_(k+1-j) * n^j see: 4乗の和，べき乗の和の公式|高校数学の美しい物語
  //      https://manabitimes.jp/math/645

  // g(n)       = -f(n) + Σ[0 <= j < n] f[j] S_j(n)
  // For t >= 1,
  // [n^t] g(n) = -f[t] + Σ[0 <= j < n] f[j] [n^t] S_j(n)
  //   = -f[t] + Σ[t-1 <= j < n] f[j]/(j+1) * (-1)^delta(t, j) * binom(j+1, t) *
  //   B_(j+1-t) = -f[t] +  1/t! * Σ[t-1 <= j < n] f[j] * (-1)^delta(t, j) * j!
  //   * B_(j+1-t)/(j+1-t)! = -f[t] + (1/t! * Σ[t-1 <= j < n] f[j] * j! *
  //   B_(j+1-t)/(j+1-t)!) - 2 * f[t] * B_1
  ...
  for (int i = 0; i <= n; ++i)
    g[i] = (ull)g[i] * inv_factorial[i] % MOD;
