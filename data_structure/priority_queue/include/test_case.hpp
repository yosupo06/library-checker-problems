#ifndef INCLUDE_TEST_CASE_PRIORITY_QUEUE
#define INCLUDE_TEST_CASE_PRIORITY_QUEUE
...
using Query = std::variant<AddQuery, PopMinQuery>;
...
TestCase random_case(Random& gen, const GeneratorParams& params) {
    ...
    for (int i = 0; i < params.q; ++i) {
        ...
        if (size_of_multiset == 0 || r < params.add_query_percent) {
            ++size_of_multiset;
            tc.add_add_query(gen.uniform(params.min_val, params.max_val));
        } else {
            --size_of_multiset;
            tc.add_min_query();
        }
...
#endif // INCLUDE_TEST_CASE_PRIORITY_QUEUE
