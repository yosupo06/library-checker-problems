...
std::vector<long long> shift(std::vector<long long> a, long long shift) {
  ...
  for (int i = 0; i < (int)a.size(); ++i)
    b[i] = (0 <= i - shift && i - shift < (int)a.size()) ? a[i - shift] : 0;
  return b;
}
...
long long inv(long long a) { return pow_mod(a, p - 2); }
...
std::vector<long long> subtract(std::vector<long long> a,
...
std::vector<long long> exp(std::vector<long long> a) {
  ...
  for (int len = 1; len < n; len *= 2) {
    std::vector<long long> tmp
        = subtract(trim(a, 2 * len), log(trim(b, 2 * len)));
    ++tmp[0];
...
void verify() {
  ...
  for (int t = 0; t < 100000; ++t) {
    ...
    for (int i = 0; i < n; ++i) { assert(u[i] == v[i]); }
