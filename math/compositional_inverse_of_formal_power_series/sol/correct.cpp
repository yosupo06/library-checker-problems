...
namespace atcoder {
    ...
    namespace internal {
        ...
        template < class T >
        using make_unsigned_int128 = typename std::conditional<
        ...
        using to_unsigned = typename std::conditional<
            ...
            typename std::conditional<
                std::is_signed< T >::value, std::make_unsigned< T >,
...
namespace atcoder {
    ...
    struct static_modint: internal::static_modint_base {
       ...
       public:
        ...
        template < class T,
                   internal::is_unsigned_int_t< T > * = nullptr >
        static_modint(T v) {
...
namespace atcoder {
    ...
    namespace internal {
        ...
        struct fft_info {
            static constexpr int rank2 =
                bsf_constexpr(mint::mod() - 1);
            std::array< mint, rank2 + 1 > root; // root[i]^(2^i) == 1
            std::array< mint, rank2 + 1 >
                iroot; // root[i] * iroot[i] == 1
            ...
            while (len) {
                if (len == 1) {
                    ...
                    for (int s = 0; s < (1 << (len - 1)); s++) {
                        ...
                        if (s + 1 != (1 << (len - 1)))
                            irot *=
                                info.irate2[bsf(~(unsigned int)(s))];
                    }
                    len--;
                } else {
                    int p     = 1 << (h - len);
                    ...
                    for (int s = 0; s < (1 << (len - 2)); s++) {
                        ...
                        for (int i = 0; i < p; i++) {
                            auto a0 =
                                1ULL * a[i + offset + 0 * p].val();
                            auto a1 =
                                1ULL * a[i + offset + 1 * p].val();
                            auto a2 =
                                1ULL * a[i + offset + 2 * p].val();
                            auto a3 =
                        ...
                        if (s + 1 != (1 << (len - 2)))
                            irot *=
                                info.irate3[bsf(~(unsigned int)(s))];
                    }
                    len -= 2;
                }
            }
    ...
    std::vector< long long > convolution_ll(
        ...
        for (int i = 0; i < n + m - 1; i++) {
            ...
            x += (c3[i] * i3) % MOD3 * M1M2;
            long long diff = c1[i] -
                internal::safe_mod((long long)(x), (long long)(MOD1));
            if (diff < 0) diff += MOD1;
...
poly operator+(const poly &x, const poly &y) {
    ...
    for (int i = 0; i < (int)y.size(); ++i) res[i] += y[i];
    return res;
}
...
poly diff(const poly &x) {
    int n = size(x);
    poly res(n - 1);
    for (int i = 1; i < n; ++i) res[i - 1] = x[i] * i;
    return res;
}

poly pow(const poly &f) {
    int n = f.size();
    vector< mint > res(n);
    for (int i = 0; i < n; ++i) {
        res[i] = f[i] * f[i];
...
poly composition(const poly &f, const poly &g) {
...
poly compositional_inverse(poly f) {
    ...
    assert(f[1] != 0);
    int n  = f.size();
    poly g = poly{0}, f_diff = diff(f);
    for (int i = 1; i < n; i *= 2) {
        g.resize(i * 2);
        g += (poly{0, 1} - composition(pre(f, i * 2), g)) *
            inv(composition(pre(f_diff, i * 2), g));
        g.resize(i * 2);
    }
    return pre(g, n);
}
