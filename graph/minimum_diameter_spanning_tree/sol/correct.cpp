...
struct SSSPNode
{
    ...
    bool operator<(const SSSPNode &t) const 
    { 
        // To maintain stability.
        if (d == t.d) return x == t.x ? y < t.y : x < t.x;
        return d > t.d; 
    }
};
...
pair<u64, vec<edge>> mdst(u32 n, vec<vec<edge>> g)
{
    ...
    for (u32 i = 0; i < n; i++) {
        ...
        // Use stable sort to avoid different performance in different platform
        // or it will cause different hash value.
        stable_sort(rnk[i].begin(), rnk[i].end(), 
