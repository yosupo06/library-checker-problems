...
int main(int, char *argv[]) {
  ...
  const auto add_edge = [&](int x, int y) { edge_buf.emplace_back(x, y); };

  const int n = gen.uniform<int>(N_MIN, 50);
  ...
  while (c.size() != c2_lim) {
    ...
    vector<int> sum;
    int cn = 2;
    while (c.size() - cn + 1 != c2_lim && gen.uniform01() < 0.7) {
      cn += 1;
    }
  ...
  auto pred = [&](auto e) { return e.first == e.second; };
  edges.erase(remove_if(edges.begin(), edges.end(), pred), edges.end());
  for (auto &e : edges)
    if (e.first > e.second)
