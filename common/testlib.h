...
const char* latestFeatures[] = {
                          ...
                          "Now ncmp.cpp and wcmp.cpp are return WA if sol is suffix or prefix of the output",
...
struct InStream
{
     ...
     * input/sol streams replace any result to FAIL.
     */
...
NORETURN void __testlib_help()
{
    ...
    std::fprintf(stderr, "    <input-file> <output-file> <sol-file> [<report-file> [<-appes>]]\n\n");
...
void registerInteraction(int argc, char* argv[])
{
    ...
    if (argc < 3 || argc > 6)
    {
        quit(_fail, std::string("Program must be run with the following arguments: ") +
            std::string("<input-file> <output-file> [<sol-file> [<report-file> [<-appes>]]]") +
    ...
    if (argc == 6)
    {
        ...
        {
            quit(_fail, std::string("Program must be run with the following arguments: ") +
                        "<input-file> <output-file> <sol-file> [<report-file> [<-appes>]]");
        }
        else
        {
            resultName = argv[4];
