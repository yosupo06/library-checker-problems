...
@{lang.en}
You are given a weighted undirected graph consisting of $N$ vertices and $M$ edges. The $i$-th edge connects vertex $u_i$ and $v_i$ and has a weight of $c_i$. This graph may not be simple.
...
@{lang.ja}
$N$ 頂点 $M$ 辺の重み付き無向グラフが与えられる。 $i$ 番目の辺は頂点 $a_i$ と $b_i$ を結んでいて、重さは $c_i$ です。このグラフは単純とは限りません。
